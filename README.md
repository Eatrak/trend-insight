...
- `topic.matched.posts`: Matched posts of topics.
- `topic.enriched.matched.posts`: Matched posts of topics with sentiment attribute.
- `topic.backfilling.tasks`: Backfilling tasks for topics.

There are three consumer groups:

- `backfill-topic`: consumers (Ingestion Service) that performs topic backfilling.
- `enrich-matched-posts`: consumers (Spark) that enriches matched posts with sentiment attribute.
